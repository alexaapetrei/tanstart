...
export default defineWebSocket({
	...
	message(peer, message) {
		...
		if (type === "join") {
			...
			peer.publish(
				roomId,
				JSON.stringify({ type: "state", state: rooms[roomId] }),
			);
			peer.send(
				JSON.stringify({
					type: "state",
			...
			if (roomId && rooms[roomId]?.players[peer.id]) {
				rooms[roomId].players[peer.id].vote = vote ?? null;
				peer.publish(
					roomId,
					JSON.stringify({ type: "state", state: rooms[roomId] }),
				);
			...
			if (roomId && rooms[roomId]?.players[peer.id]?.isGM) {
